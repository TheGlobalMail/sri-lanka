...
if [ $target == "staging" ] ; then
  ...
  s3cmd -P -m text/html put dist/family.html s3://sri-lanka-staging.theglobalmail.org/family
  grunt clean:dist
fi
if [ $target == "production" ] ; then
  ...
  s3cmd -P -m text/html put dist/family.html s3://sri-lanka.theglobalmail.org/family
  grunt clean:dist
fi